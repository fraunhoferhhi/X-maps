...
def first_event_per_xy(events):
    ...
    print(f"Filtered {len(events)} events to first per cam x/y: {len(filtered_events)} events.")

    return filtered_events


@benchmark
def first_event_per_yt(events, xp_i16):
    events = events[events["p"] == 1]
    ...
    print(f"Filtered {len(events)} events to first per t/y: {len(filtered_events)} events.")

    return filtered_events


@benchmark
def mean_first_last_event_per_xy(events):
    events = events[events["p"] == 1]

    # TODO perf not optimized
    first_event_map = np.zeros((events["y"].max() + 1, events["x"].max() + 1), dtype=np.int32)
    event_mask = np.zeros_like(first_event_map, dtype=np.bool)
